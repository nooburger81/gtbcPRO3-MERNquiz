...
module.exports.loginValidator = (data) => {
    ...
    data.password = !(isEmpty(data.password))? data.password : '';
     ...
     let emailError = validator.isEmpty(data.email)? 'Email is required' : (!validator.isEmail(data.email) ? 'Email is invalid' : '');
...
module.exports.registerValidator = (data) => {
    ...
    data.lastName = !(isEmpty(data.lastName))? data.lastName : '';
     ...
     let firstNameError = validator.isEmpty(data.name)? 'Name is required' : '';
     let emailError = validator.isEmpty(data.email)? 'Email is required' : (!validator.isEmail(data.email)? 'Email is invalid' : '');
     let passwordError = validator.isEmpty(data.password)? 'Password is required' : '';
